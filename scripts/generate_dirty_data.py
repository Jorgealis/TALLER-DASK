...
Generador sintetico de 300.000 registros con anomalias severas de calidad de datos.
Basado en la guia del taller: raw_customer_code heterogeneo, mojibake UTF-8/Latin-1
en city_notes_corrupted, telefonos caoticos y nulos distribuidos.
"""
import os
import random
import numpy as np
...
def generate_dirty_dataset(num_rows=300_000, output_dir="/shared-data/raw"):
    ...
    print(f"[*] Iniciando generacion de {num_rows:,} filas con ruido sintetico...")
    ...
    # Patron esperado: codigo numerico de 5 digitos inmerso en ruido
    ...
    # OJO: se muestrea una sola vez por fila (una plantilla) y se reusa,
    # en vez de llamar random.choices() dos veces (bug: podian salir dos
    # plantillas distintas -> intentar .format() sobre None).
    raw_customer_code = []
    for cid in clean_ids:
        template = random.choices(templates, weights=weights)[0]
        raw_customer_code.append(template.format(id=cid) if template is not None else np.nan)
    ...
    # 3. Columna de telefonos caoticos
    phone_templates = [
        "+57 (310) {p1}-{p2}",
        "310.{p1}.{p2}",
    ...
    for idx in range(num_chunks):
        ...
        print(f"  -> Generada particion {idx + 1}/{num_chunks}: {chunk_path} ({len(chunk_df):,} filas)")

    print(f"[OK] Conjunto de datos sintetico generado exitosamente en {output_dir}/")
